...
将 [`examples/codex-config.toml`](examples/codex-config.toml) 加入 `%USERPROFILE%/.codex/config.toml`，替换 Node.js 和 launcher 的绝对路径。运行 `powershell -File examples/create-auth.ps1`，在 `%LOCALAPPDATA%/scrooge-mcp/auth.dpapi` 创建当前 Windows 用户专属的加密凭据。
...
8. 配置完成后，提醒用户运行 `powershell -File examples/create-auth.ps1`，在 `%LOCALAPPDATA%/scrooge-mcp/auth.dpapi` 创建加密凭据；不要把 key 放入 task、工具参数、日志或回复。
9. 不要声称缓存命中率或费用是精确值，除非服务返回对应计量字段。
```

## English Usage

Node.js `22.19.0+` is required. Install DSH, run `npm install`, then `npm run build`. Create the encrypted credential with `powershell -File examples/create-auth.ps1`. Copy [`examples/codex-config.toml`](examples/codex-config.toml) into `%USERPROFILE%/.codex/config.toml`, replace paths, and set `SCROOGE_AUTH_FILE` to the `.dpapi` file. Never commit the auth file.
...
8. After configuring MCP, tell the user to run `powershell -File examples/create-auth.ps1`; this creates the current-user encrypted credential at `%LOCALAPPDATA%/scrooge-mcp/auth.dpapi`. Never put the key in task text, tool arguments, logs, or replies.
